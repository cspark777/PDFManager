...
#ifndef _BNB_ASSISTANT
	if(m_nTotalFileCount >= g_nMaximumFileCount)
		return 0;
#endif
	...
	if (opts.bSubfolders)
	{
		...
		while ((bFound) && (!_bStopSearch))
		{
			...
			if(finder.IsDirectory())
			{
				...
				for(POSITION pos = opts.sExludedDirs.GetHeadPosition();pos;)
					if(finder.GetFileName().Find(opts.sExludedDirs.GetNext(pos)) == 0) {
						bExcluded = TRUE;
						break;
					}
