...
LRESULT CALLBACK CShellContextMenu::HookWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	// With this code not blank menu showed
...
UINT CShellContextMenu::ShowContextMenu(CWnd *pWnd, CPoint pt, CMenu* inParentMenu, LPCTSTR inParentName, int inPos)
{
	...
	{
		inParentMenu->InsertMenu(inPos, MF_BYPOSITION | MF_POPUP, (UINT)m_Menu->m_hMenu, inParentName);
		lMenu = inParentMenu;
	}
	else
		lMenu = m_Menu;
	...
	UINT idCommand = lMenu->TrackPopupMenu (TPM_RETURNCMD | TPM_LEFTALIGN, pt.x, pt.y, pWnd);
...
void CShellContextMenu::SetObjects(CStringArray &strArray)
{
	...
	free (olePath);
#else
	CString strFunction = strArray.GetAt(0);
	BSTR b = strFunction.AllocSysString();
	LPOLESTR strFxn = b;

	psfDesktop->ParseDisplayName (NULL, 0, /*strArray.GetAt(0).GetBuffer(0)*/strFxn, NULL, &pidl, NULL);
	SysFreeString(b);
#endif
