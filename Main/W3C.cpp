...
#define __DEFAULT_BUF_SIZE 1024
	...
	void __w3cexcept(const char *szaddress, long nport, CW3Client::w3t t, const char *szmsg){
#ifdef _DEBUG
		string sztmp;
		sztmp+="[ ";
		switch(t) {
		case CW3Client::w3http:
			sztmp+="http://";
			break;
		case CW3Client::w3https:
			sztmp+="https://";
			break;
		case CW3Client::w3ftp:
			sztmp+="ftp://";
			break;
		}
		sztmp+=szaddress;
	...
	void __w3cexcept(const char *szaddress, long nport, CW3Client::w3t t, const char *szuri, const char *szmsg){
#ifdef _DEBUG
		string sztmp;
		sztmp+="[ ";
		switch(t) {
	...
	// CW3Client : synchronized www client
	bool CW3Client::Connect(const char *szaddress,
	...
	bool CW3Client::Request(const char *szuri, w3m m, const char *szref /*=NULL*/){
	...
	bool CW3Client::RequestGet(const char *szuri, const char *szref /*=NULL*/){

		static const char* szAcceptType= __HTTP_ACCEPT_TYPE;
	...
	void CW3Client::InitializePostArguments(){
	...
	void CW3Client::AddPostArgument(const char *szname, const int nvalue){
		HTTP_ARG *pa=new HTTP_ARG(szname, nvalue);
		_listargs.push_back(pa);
		return;
	}

	void CW3Client::AddPostArgument(const char *szname, const long nvalue){
		HTTP_ARG *pa=new HTTP_ARG(szname, nvalue);
		_listargs.push_back(pa);
		return;
	}

	void CW3Client::AddPostArgument(const char *szname, const float nvalue){
		HTTP_ARG *pa=new HTTP_ARG(szname, nvalue);
		_listargs.push_back(pa);
		return;
	}

	void CW3Client::AddPostArgument(const char *szname, const double nvalue){
		HTTP_ARG *pa=new HTTP_ARG(szname, nvalue);
		_listargs.push_back(pa);
		return;
	}

	void CW3Client::AddPostArgument(const char *szname, const char *szvalue, bool bfile /*=false*/){
		HTTP_ARG *pa=new HTTP_ARG(szname, szvalue, bfile);
		_listargs.push_back(pa);
		return;
	}

	void CW3Client::InitializeCookies(){
		if(_listcookies.size()>0){
			for(list<HTTP_COOKIE*>::iterator it=_listcookies.begin(); it!=_listcookies.end(); it++){
		...
		void CW3Client::AddCookie(const char *szname, const char *szvalue){
	...
	void CW3Client::AddCookie(const char *szname, const int value){
		HTTP_COOKIE *pc=new HTTP_COOKIE(szname, value);
		_listcookies.push_back(pc);
		return;
	}

	void CW3Client::AddCookie(const char *szname, const long value){
		HTTP_COOKIE *pc=new HTTP_COOKIE(szname, value);
		_listcookies.push_back(pc);
		return;
	}

	void CW3Client::AddCookie(const char *szname, const float value){
		HTTP_COOKIE *pc=new HTTP_COOKIE(szname, value);
		_listcookies.push_back(pc);
		return;
	}

	void CW3Client::AddCookie(const char *szname, const double value){
		HTTP_COOKIE *pc=new HTTP_COOKIE(szname, value);
		_listcookies.push_back(pc);
		return;
	}
	...
	unsigned long CW3Client::GetPostArgumentsLength(){
		unsigned long len=0;
		if(_listargs.size()>0){
			for(list<HTTP_ARG*>::iterator it=_listargs.begin(); it!=_listargs.end(); it++){
				HTTP_ARG *p=reinterpret_cast<HTTP_ARG*>(*it);
	...
	unsigned long CW3Client::GetPostData(unsigned char *buf, unsigned long len){
	...
	bool CW3Client::RequestPost(const char *szuri, const char *szref /*=NULL*/){
		...
		// REPLACE HEADER
		if(!::HttpAddRequestHeadersA( _hRequest, __HTTP_ACCEPT, (DWORD)strlen(__HTTP_ACCEPT), HTTP_ADDREQ_FLAG_REPLACE))
			throw "additional header failed...";
	...
	unsigned long CW3Client::GetMultiPartsFormDataLength(){
		unsigned long len=0;
		if(_listargs.size()>0){
			for(list<HTTP_ARG*>::iterator it=_listargs.begin(); it!=_listargs.end(); it++){
				HTTP_ARG *p=reinterpret_cast<HTTP_ARG*>(*it);
	...
	unsigned long CW3Client::AllocMultiPartsFormData(unsigned char *&buf, const char *szboundry){
	...
	void CW3Client::FreeMultiPartsFormData(unsigned char *buf){ ::HeapFree(::GetProcessHeap(), 0, buf); return; }

	bool CW3Client::RequestPost2(const char *szuri, const char *szref /*=NULL*/){
		static const char* szAcceptType=__HTTP_ACCEPT_TYPE;
		static const char* szContentType="Content-Type: multipart/form-data; boundary=--MULTI-PARTS-FORM-DATA-BOUNDARY\r\n";
		...
		if(!::HttpAddRequestHeadersA( _hRequest, szContentType, (DWORD)strlen(szContentType), HTTP_ADDREQ_FLAG_ADD_IF_NEW))
			throw "additional header failed...";
	...
	unsigned long CW3Client::QueryCookie(unsigned char *buf, unsigned long len, unsigned long idx /*=0*/){
	...
	unsigned long CW3Client::QueryContentLength(){
	...
	const char * CW3Client::QueryContentType(){
	...
	unsigned int CW3Client::QueryResult() {
		char szt[__DEFAULT_BUF_SIZE]="\0";
		unsigned long nread=__DEFAULT_BUF_SIZE;
		memset(szt, 0x00, __DEFAULT_BUF_SIZE);		
	...
	unsigned long CW3Client::QueryRawHeader(unsigned char *buf, unsigned long len){
	...
	unsigned long CW3Client::Response(unsigned char *buf, unsigned long len){
	...
	bool CW3Client::GetFile(const char *szuri, const char *szfile, bool ascii /*=false*/){
	...
	unsigned long CW3Client::GetFile(const char *szuri, unsigned char *buf, unsigned long len, bool ascii /*=false*/){
	...
	bool CW3Client::PutFile(const char *szuri, const char *szfile, bool ascii /*=false*/){
	...
	unsigned long CW3Client::PutFile(const char *szuri, unsigned char *buf, unsigned long len, bool ascii){
	...
	bool AsyncW3Client::RequestGet(const char *szuri, const char *szref /*=NULL*/){
		...
		// REPLACE HEADER
		if(!::HttpAddRequestHeadersA( _hRequest,
														__HTTP_ACCEPT, (DWORD)strlen(__HTTP_ACCEPT),														
														HTTP_ADDREQ_FLAG_REPLACE))
			throw "additional header failed...";
	...
	bool AsyncW3Client::RequestPost(const char *szuri, const char *szref /*=NULL*/){
		...
		InternetBufferIn.dwHeadersLength=(DWORD)strlen(szContentType);
		...
		if(!::HttpEndRequestA(_hRequest, NULL, HSR_ASYNC | HSR_INITIATE, reinterpret_cast<unsigned long>(this))
			&&
			::GetLastError()!=ERROR_IO_PENDING
			){
			// free
	...
	bool AsyncW3Client::RequestPost2(const char *szuri, const char *szref /*=NULL*/){
		...
		if(!::HttpAddRequestHeadersA( _hRequest, szcl, (DWORD)strlen(szcl), HTTP_ADDREQ_FLAG_ADD_IF_NEW))
			throw "additional header failed...";		
