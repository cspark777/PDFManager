 ...
 * ---------------------------------------------------------------------------------------
 * description :
 *    CW3Client ; synchronized www client class
...
#define __SIZE_HTTP_HEAD_LINE	2048
	...
	private:
		typedef struct __HTTP_ARG {
			...
			const char *type(){
				...
				for(ndot=(long)name.length(); ndot>0; ndot--)
			...
			unsigned long length(){	return (unsigned long)name.length()+(unsigned long)value.length()+1; }
			unsigned long dump(unsigned char *buf, unsigned long s){
				...
				return (unsigned long)name.length()+value.length()+1;
			}
			...
			unsigned long dump2(unsigned char *buf, unsigned long s, const char *boundry=__DEFAULT_BOUNDRY_TAG){
				...
				if(f){ // file
					...
					*/
					...
					l=(unsigned long)(2+strlen(boundry)+2+38+name.length()+13+value.length()+5);
					...
					sprintf( reinterpret_cast<char*>(buf),
								...
								"%s\r\n",
								boundry, name.c_str(), value.c_str());
					l=(unsigned long)(2+strlen(boundry)+2+38+name.length()+5+value.length()+2);
				}
				return l;
			}			
		} HTTP_ARG, *PHTTP_ARG;
	...
	public:
		CW3Client(){ _hOpen=NULL; _hConnection=NULL, _hRequest=NULL; }
		virtual ~CW3Client(){ InitializePostArguments(); InitializeCookies();}
	public:		
	...
	class AsyncW3Client : public CW3Client {
	public:
		AsyncW3Client():CW3Client(){_hCompleteRequestEvent=NULL;}
	public:
		bool Connect(const char *szaddress,
		...
		bool Request(const char *szuri, w3m m=reqGet, const char *szref=NULL){
			...
			return CW3Client::Request(szuri, m, szref);
		}
		unsigned long Response(unsigned char *buf, unsigned long len){
			::CloseHandle(_hCompleteRequestEvent);
			_hCompleteRequestEvent=NULL;
			return CW3Client::Response(buf, len);
		}
	public:
